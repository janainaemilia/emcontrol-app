...
class VideoBuilder {
  ...
  buildNode(node, wrapper) {
    ...
    setTimeout(() => {
      ...
      let iframe = node.getElementsByTagName('video')[0];
      // let iframe = node.getElementsByTagName('video')[0];
      iframe.setAttribute('width', node.getAttribute('width') || 300);
      iframe.setAttribute('height', node.getAttribute('height') || 150);
    }, 0);
  ...
  select(overlay, quill, node) {
    ...
    if (this.selectedElement.className.indexOf('active') === -1){
      ...
      this.node = node;
      this.iframe = this.parentElement.getElementsByTagName('video')[0];
...
class Video extends BlockEmbed {
  ...
  static formats(domNode) {
    let iframe = domNode.getElementsByTagName('video')[0];
  ...
  static value(domNode) {
    return domNode.getElementsByTagName('video')[0].getAttribute('src');
  }
